// src/components/HistoryGrid.jsx
...
export default function HistoryGrid({
    ...
    const load = async () => {
      ...
      try {
        const res = await api.get(
          `/videos?viewedBy=${encodeURIComponent(user._id)}`,
        );
        const payload = res?.data?.videos ?? [];
        if (mounted) setVideos(payload);
      } catch (err) {
        console.warn("HistoryGrid load failed:", err);
  ...
  if (loading)
    return <div className="bg-white p-4 rounded shadow">Loading history…</div>;
  if (!videos.length)
    return (
      <div className="bg-white p-4 rounded shadow">No watched videos yet.</div>
    );
