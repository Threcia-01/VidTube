// src/components/YourVideosGrid.jsx
import React, { useEffect, useState, useContext } from "react";
import { youtubeContext } from "../contexts/vidtubeContext";

export default function YourVideosGrid({
  api: apiProp,
  user,
  onVideoClick = () => {},
}) {
    ...
    const load = async () => {
      ...
      try {
        const res = await api.get(
          `/videos?userId=${encodeURIComponent(user._id)}`,
        );
        const payload = res?.data?.videos ?? [];
        if (mounted) setVideos(payload);
      } catch (err) {
        console.warn("YourVideosGrid load failed:", err);
  ...
  if (loading)
    return <div className="bg-white p-4 rounded shadow">Loading videos…</div>;
  if (!videos.length)
    return (
      <div className="bg-white p-4 rounded shadow">No videos to show.</div>
    );
